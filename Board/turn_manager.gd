...
func _ready() -> void:
	...
	turn_order.sort_custom(func(a, b):
		return a.unit_data.initiative > b.unit_data.initiative
	)
	for unit in turn_order:
		var mini_scene = load("res://Board/Interface/character_min.tscn")
		var miniature = mini_scene.instantiate()
		miniature.name = "%s_mini" % unit.name
		miniature.unit_name = unit.name
		miniature.artwork = load("res://Board/Unit/Player/artwork.png")
		$Interface/Initiative/HBoxContainer.add_child(miniature)
	turn_order[0].turn_started.emit()
